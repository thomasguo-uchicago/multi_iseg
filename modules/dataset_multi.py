...
class EncoderDataset(Dataset):
    def __init__(self, data_dir, mesh, args):

        _, self.training_inds = fps_from_given_pc(pts=mesh.vertices, k=round(args.data_percentage*mesh.vertices.shape[0]), given_pc=mesh.vertices[0])
    ...
    def _load_data(self, file_path):
        ...
        with torch.no_grad():            
            ...
            if self.args.return_original == 1:
                ...
                image_path = os.path.join(self.args.dataset_path, '{}/{}/target_save_{}_grey.png'.format(self.mesh.name, selected_vertices[0], file['view_ind']))
